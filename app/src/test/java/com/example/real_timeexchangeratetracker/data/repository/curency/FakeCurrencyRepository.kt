...
open class FakeCurrencyRepository : CurrencyRepository {
    ...
    override suspend fun updateCurrency(currency: Currency) {
        ...
        if (currency.isMonitored) {
            if (indexMonitored == -1) currentMonitored.add(currency) else currentMonitored[indexMonitored] = currency
        } else {
    ...
    override fun searchCurrencies(query: String): Flow<List<Currency>> {
        ...
        searchResultsFlow.value = allCurrenciesFlow.value.filter {
            it.shortName.contains(query, ignoreCase = true) || it.longName.contains(query, ignoreCase = true)
        }
        return searchResultsFlow
    }

    fun setAllCurrencies(currencies: List<Currency>) {
        allCurrenciesFlow.value = currencies
    }
